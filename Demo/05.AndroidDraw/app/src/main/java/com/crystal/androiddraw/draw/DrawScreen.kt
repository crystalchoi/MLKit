...
fun AndroidDrawView() {
    ...
    var textResultState: String by remember { mutableStateOf("...") }

    lateinit var drawViewForDigit: DrawView

    ConstraintLayout(modifier = Modifier.offset(x = 0.dp, y = 0.dp)) {
        val (drawViewRef, button, textResult) = createRefs()

        Column(modifier = Modifier.offset(x = 0.dp, y = 0.dp).height(IntrinsicSize.Max)
